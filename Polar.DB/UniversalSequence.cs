...
namespace Polar.DB
{
    ...
    /// </summary>
    public class UniversalSequence<T> where T : IComparable
    {
        private PType tp_elem; // Поляровский тип элемента
        private Stream fs; // Стрим - среда для последовательности. Сначала 8 байтов длина, потом подряд бинарные развертки элементов 
        private BinaryReader br;
        private BinaryWriter bw;
        private int elem_size = -1; // длина элемента, если она фиксирована, иначе -1
        private long nelements; // текущее количество элеметов. В "покое" - совпадает со значением в первых 8 байтах 
        public UniversalSequence(PType tp_el, Stream media)
        {
            tp_elem = tp_el;
